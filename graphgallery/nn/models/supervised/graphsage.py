...
class GraphSAGE(SupervisedModel):
    ...
    def __init__(self, adj, x, labels, n_samples=[15, 5], norm_x_type='row_wise', 
    ...
    def preprocess(self, adj, x):
        ...
        with tf.device(self.device):
            self.tf_x, self.neighbors = self.to_tensor(x), neighbors
    ...
    def train_sequence(self, index):
        ...
        with tf.device(self.device):
            sequence = SAGEMiniBatchSequence([self.tf_x, index], labels, neighbors=self.neighbors, n_samples=self.n_samples)
        return sequence
    ...
    def predict(self, index):
        ...
        index = self.to_int(index)
        with tf.device(self.device):
            data = SAGEMiniBatchSequence([self.tf_x, index], neighbors=self.neighbors, n_samples=self.n_samples)
