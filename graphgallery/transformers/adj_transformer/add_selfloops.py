...
class AddSelfLoops(Transformer):
    ...
    def __call__(self, *adj_matrix):
        ...
        """
        return add_selfloops(*adj_matrix, selfloop=self.selfloop)
...
@MultiInputs()
def add_selfloops(adj_matrix, selfloop: float = 1.0):
    ...
    if is_list_like(selfloop):
        return tuple(_add_selfloops(adj_matrix, w) for w in selfloop)
    else:
        return _add_selfloops(adj_matrix, selfloop)
