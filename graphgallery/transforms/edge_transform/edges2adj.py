...
class SparseEdgesToSparseAdj(Transform):
    def __call__(self, edge_index: np.ndarray, edge_weight: np.ndarray = None, shape=None) -> sp.csr_matrix:
        return sparse_adj_to_sparse_edge(edge_index=edge_index, edge_weight=edge_weight, shape=shape)

    def __repr__(self):
        return f"{self.__class__.__name__}()"


def sparse_edge_to_sparse_adj(edge_index: np.ndarray, edge_weight: np.ndarray = None, shape=None) -> sp.csr_matrix:
