...
def normalize_adj_tensor(adj, rate=-0.5, self_loop=True):
    if self_loop:
...
def add_self_loop_edge(edge_index, edge_weight, n_nodes=None, fill_weight=1.0):
    ...
    diagnal_edge_weight = tf.zeros([n_nodes], dtype=self.floatx) + fill_weight
...
def normalize_edge(edge_index, edge_weight=None, n_nodes=None, fill_weight=1.0, rate=-0.5):
    ...
    if edge_weight is None:
        edge_weight = tf.ones([edge_index.shape[0]], dtype=self.floatx)
